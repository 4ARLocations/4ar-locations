    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
      <div className="mb-8">
        ...
        <p className="text-[#9B8A74] mt-2">
          {mounted && favProps.length > 0
            ? `${favProps.length} logement${favProps.length > 1 ? 's' : ''} sauvegardé${favProps.length > 1 ? 's' : ''}`
            : 'Retrouvez ici les logements que vous avez mis en favoris.'}
        </p>
      </div>
      ...
      {!mounted ? null : favProps.length === 0 ? (
        <div className="text-center py-20 bg-[#FAF7F2] rounded-2xl border border-[#E8DCC8]">
          ...
          <p className="text-[#5C4F3A] font-medium text-lg mb-2">Aucun favori pour l&apos;instant</p>
          <p className="text-[#9B8A74] text-sm mb-6">Cliquez sur le ❤️ sur un logement pour le retrouver ici.</p>
          <Link
            href={`/${locale}/biens`}
            ...
            {favProps.map((p) => (
              <div key={p.id} className="bg-white border border-[#E8DCC8] rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
                <Link href={`/${locale}/biens/${p.slug}`}>
                  ...
                  <div className="p-4">
                    <h2 className="font-bold text-[#2C2416] text-lg mb-1">{t(p.nameKey)}</h2>
                ...
                <div className="px-4 pb-4 flex items-center justify-between border-t border-[#F0EAE0] pt-3">
                  <span className="text-[#C8763A] font-bold text-lg">
                    {p.priceFrom > 0 ? `${p.priceFrom}€` : 'Sur demande'}
                    {p.priceFrom > 0 && <span className="text-xs font-normal text-[#9B8A74]">/nuit</span>}
                  </span>
                  <button
